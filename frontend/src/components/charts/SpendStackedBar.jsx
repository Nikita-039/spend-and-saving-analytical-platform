...
const CustomTooltip = ({ active, payload, label }) => {
  ...
  return (
    <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: '10px', padding: '10px 14px', fontSize: 12 }}>
      ...
      {payload.map((p, i) => (
        <p key={i} style={{ color: p.fill }}>{p.name}: {formatCurrency(p.value, true)}</p>
      ))}
    </div>
  );
};

export default function SpendStackedBarChart({ data = [] }) {
  const chartData = data.map(d => ({
    name: d._id,
    Budget: d.totalBudget,
    Spend: d.totalActualSpend,
    Savings: Math.max(0, d.totalBudget - d.totalActualSpend),
    Overspend: Math.max(0, d.totalActualSpend - d.totalBudget),
  }));
  ...
  return (
    <ResponsiveContainer width="100%" height={260}>
      <BarChart data={chartData} margin={{ top: 5, right: 10, left: 10, bottom: 5 }}>
        ...
        <Bar dataKey="Spend" stackId="a" fill={CHART_COLORS[1]} radius={[0, 0, 0, 0]} maxBarSize={50} />
        <Bar dataKey="Savings" stackId="a" fill={CHART_COLORS[2]} radius={[4, 4, 0, 0]} maxBarSize={50} />
      </BarChart>
    </ResponsiveContainer>
  );
}
